...
const OrderTableRow: FC<OrderTableRowProps> = ({ order }) => {
  return (
    <tr className="bg-white dark:bg-transparent border-b dark:border-neutral-700 hover:bg-gray-50 dark:hover:bg-neutral-700/50">
      ...
      <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white whitespace-nowrap">{order.nama}</td>
      <td className="px-6 py-4">{order.layanan}</td>
      <td className="px-6 py-4">{order.no_hp}</td>
      <td className="px-6 py-4">{order.kota}</td>
      <td className="px-6 py-4"><OrderStatusPill status={order.status_pembayaran} /></td>
